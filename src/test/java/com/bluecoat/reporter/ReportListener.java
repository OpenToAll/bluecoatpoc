...
public class ReportListener implements IReporter {
    ...
    // Get results of all suites. Assumption for is that there is only one suite
    ...
    private void writeRunParameters() {
		...
		for (ISuite suite : m_suites) {
			...
			Header += "<th>Browser</th>";
			row += "<td>" + Global.executionDetailsMap.get("Browser") + "</td>";
			noOfColumns++;
                ...
                for (MethodResult methodResult : classResult.getMethodResults()) {
    				...
    				if( null != testMthdDetail) {
    					pathToScreenShot = suiteName + "/"
    						+ firstResult.getMethod().getMethodName() + ".jpeg";
    					
    					pathToHTMLSourceFile = suiteName + "/"
        						+ firstResult.getMethod().getMethodName() + ".html";
    				
    					pathToLogFile = dirPathToLogs + "/"
    							+ firstResult.getMethod().getMethodName() + ".log"; 	
    					
    					detailTableID = firstResult.getName();
    				}
