...
This class file implements all required methods for excel sheets.
 * The following methods are implememnted
 * findSheet
 * getRowCount
 * getColumnCount
 * getCellData
...
public class ExcelReader {
	...
	protected static XSSFSheet sheet = null;
	...
	// verifies whether the specified sheet is available in the excel or not
	public boolean findSheet(String sheetName) {
		int numberOfSheets = workbook.getNumberOfSheets();
		String strSheetName;
		for (int i = 0; i < numberOfSheets; i++) {
			strSheetName = workbook.getSheetName(i);
			if (sheetName.equalsIgnoreCase(strSheetName)) {
				return true;
			}
		}
		return false;
	...
	public XSSFRow getRow(String sheetName, int rowNum) {
        int index = workbook.getSheetIndex(sheetName);
        XSSFRow rw=workbook.getSheet(sheetName).getRow(rowNum);
        if (rw != null)
              return rw;
        else {
              return null;
...
public XSSFWorkbook getWorkbook() {
	...
	public String getCellData(String sheetName, String colName, int rowNum) {
		try {
			...
			if (cell.getCellType() == Cell.CELL_TYPE_STRING) {
				...
				String cellText = String.valueOf(cell.getRawValue());// .getNumericCellValue()
	...
	public boolean setCellData(String sheetName, String colName, int rowNum, String data) {
		try {
			...
			row = sheet.getRow(0);
			System.out.println("The last row number is " + row.getLastCellNum());
			for (int i = 0; i < row.getLastCellNum(); i++) {
				// System.out.println(row.getCell(i).getStringCellValue().trim());
				if (row.getCell(
					i).getStringCellValue().trim().equals(
					colName))
					colNum = i;
			}
			if (colNum == -1)
	...
	public boolean setCellData(String sheetName, int colNum, int rowNum, String data) {
		try {
			 ...
			 * row = sheet.getRow(0);
			 * //System.out.println(row.getLastCellNum()); for (int i = 0; i <
			 * row.getLastCellNum(); i++) {
			 * //System.out.println(row.getCell(i).getStringCellValue().trim());
			 * if (row.getCell( i).getStringCellValue().trim().equals( colName))
			 * colNum = i; } if (colNum == -1) return false;
	...
	public boolean setCellData(String sheetName, String colName, int rowNum, String data, String url) {
		...
		try {
			...
			for (int i = 0; i < row.getLastCellNum(); i++) {
				// System.out.println(row.getCell(i).getStringCellValue().trim());
				if (row.getCell(
					i).getStringCellValue().trim().equalsIgnoreCase(
					colName))
					colNum = i;
			}
